...
public final class OpenEsbMWorkspaceParser extends AbstractParser {
	...
	public final OpenEsbMultiWorkspace parse(File workspaceFolder) throws ServiceParserException {
		...
		OpenEsbMultiWorkspace multiWorkspace = new OpenEsbMultiWorkspace("", workspaceFolder);
		...
		parseProjectSources(esbBpelProjects);
	 ...
	 */
	private final void parseProjectSources(List<OpenEsbBpelProject> esbBpelProjects) {
		for (OpenEsbBpelProject project : esbBpelProjects) {
			List<File> bpelFiles = new ArrayList<File>();
			filterBpelProcess(new File(project.getFolder() + File.separator + "src"), bpelFiles);
